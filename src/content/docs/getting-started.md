...
4. From the drop down menu, choose your AI provider. Jarvix supports OpenAI, Anthropic, Gemini and OpenRouter. The latter opens up virtually all LLM providers for you.
...
If you encounter any issues with the authentication process feel free to [Contact Us](https://jarvix.site/contact-us).

## Exploring Jarvix
Jarvix is made up of 5 main sections; the Pipeline Status bar at the top, the Agents pane, the Pipeline Graph, the Agent Details pane and the Activity Log. 
...
> Note that [agents](agents.md)' order in Jarvix is important. A review [agents](agents.md) (an [agents](agents.md) that reviews work of another [agents](agents.md)) can not exist before the [agents](agents.md) doing the work.
...
### Activity Log
Here is where Jarvix will communicate with you; it shows helpful messages that portray the state of the pipeline. From when an [agents](agents.md) starts to when an error occurs and what kind of error is it. If something is wrong with your pipeline, this is the first place to check. 
